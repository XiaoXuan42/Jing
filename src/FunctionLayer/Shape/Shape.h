...
public:
    ...
    virtual void fillIntersection(float distance, int primID, float u, float v,
                                  SurfaceIntersection *intersection) const = 0;

    //* 使用一个二维随机数在shape上进行均匀随机采样
    //* 采样点的信息（位置，法线等）存储在result中，方便起见我们使用Intersection结构而不创建新的数据结构（后续可以改进）
    //* 采样结果的pdf是area measure
    virtual void uniformSampleOnSurface(Vector2f sample, SurfaceIntersection *result,
                                        float *pdf) const = 0;
...
public:
    ...
    std::unique_ptr<Medium> medium_outside, medium_inside;

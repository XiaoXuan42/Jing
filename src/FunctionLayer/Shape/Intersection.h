...
inline void computeRayDifferentials(SurfaceIntersection *intersection,
    ...
    do {
        if (ray.hasDifferentials) {
            ...
            Vector3f oy =
                Vector3f{ray.originY[0], ray.originY[1], ray.originY[2]};
            float d = dot(n, Vector3f{p[0], p[1], p[2]});
            float tx = -(dot(n, ox) - d) / dot(n, ray.directionX);
            if (std::isinf(tx) || std::isnan(tx)) break;
            float ty = -(dot(n, oy) - d) / dot(n, ray.directionY);
            if (std::isinf(ty) || std::isnan(ty)) break;
            ...
            float dudx, dvdx, dudy, dvdy;
            if (!solveLinearSystem2x2(A, Bx, &dudx, &dvdx)) dudx = dvdx = .0f;
            if (!solveLinearSystem2x2(A, Bx, &dudy, &dvdy)) dudy = dvdy = .0f;

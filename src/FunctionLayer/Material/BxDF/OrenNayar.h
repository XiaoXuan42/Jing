...
public:
    ...
    virtual Spectrum f(const Vector3f &wo, const Vector3f &wi) const override {
        ...
        float alpha = std::max(theta_o, theta_i),
              beta = std::min(theta_o, theta_i);
        float A = 1 - sigma * sigma / (2 * (sigma * sigma + 0.33));
        float B = 0.45 * sigma * sigma / (sigma * sigma + 0.09);
        float phi_i = atan2(wiLocal[0], wiLocal[2]),
              phi_o = atan2(woLocal[0], woLocal[2]);
        float coeff = 1.0 / M_PI *
                      (A + B * std::max(0.0f, std::cos(phi_i - phi_o)) *
                               std::sin(alpha) * std::tan(beta));
